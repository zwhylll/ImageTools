...
from controllor.CompressPicture import *
# import requests
...
class Win_ImageZoom:
    ...
    def __init__(self):
        ...
        self.ui = QUiLoader().load('ImageZoom.ui')
    ...
    def getCompressResolution(self):
        #compressL = self.ui.LText.text()
        #compressW = self.ui.WText.text()
        #result = compressL+"*"+compressW
        s5 = self.ui.formImageBox.currentText()
        return float(s5)
        #print(compresslevel)
    ...
    def compressImage(self):
       ...
       # s3 = compresslevel.split("*")[-1]
        ...
        if (len(road) == 0 or len(infileName) == 0):
            ...
            wlog(infileName, outfileName, "图像缩放")
            compressPicZoom(infileName, outfileName,compresslevel )
            self.msg1()
    def msg1(self):
        # 使用infomation信息框
